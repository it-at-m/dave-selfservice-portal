...
import {CircleMarker, CircleMarkerOptions, control, Icon, LatLng, MapOptions, Marker, MarkerOptions} from "leaflet"
...
export default class ZaehlungCardMap extends Vue {
  ...
  private createMarkerForZaehlstelle(coords: LatLng) {
    ...
    this.markerZaehlstelle = new CircleMarker(
        coords,
        options
    );

    this.markerZaehlstelle.bindTooltip('Zählstelle',
        {direction: "right", offset: [options.radius, 0]});
  ...
  private createMarkerForZaehlung(coords: LatLng) {
    ...
    this.markerZaehlung = new Marker(
        coords,
        options
    );
