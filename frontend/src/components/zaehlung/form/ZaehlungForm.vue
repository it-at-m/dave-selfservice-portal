  ...
  >
    ...
    >
      ...
      <v-tab-item ref="allgemeineInfo">
        <allgemeine-info-form :height="SHEETHEIGHT" @isValid="setAllgemeineFormValid"/>
      </v-tab-item>
...
import {intervallnummern, StartUhrzeitEndeUhrzeit} from "@/domain/enums/Intervallnummern";
...
export default class ZaehlungForm extends Vue {
  ...
  private isAllgemeinFormValid: boolean = false;
  ...
  private save(): void {
    ...
    ZaehlungService.saveZaehlung(copy).then((savedDTO: SavedDTO) => {
  ...
  private prepareForSaveZaehlung(zaehlung: ZaehlungDTO) {
    const zeitintervalleProFahrbeziehung: Map<string, Array<ZeitintervallDTO>> = new Map<string, Array<ZeitintervallDTO>>();
    zaehlung.knotenarme.forEach((arm: KnotenarmDTO) => {
      if (arm.filename && arm.filedata && arm.filedata.length > 0) {
        this.transformCsvDataToFahrbeziehung(arm).forEach((value, key) => {
    ...
    zaehlung.fahrbeziehungen.forEach((fz: FahrbeziehungDTO) => {
      const key: string = this.getKeyOfFahrbeziehung(fz, zaehlung.kreisverkehr);
  ...
  private getStartEndeOfIntervallnummer(nummer: string): StartUhrzeitEndeUhrzeit {
   ...
   */
  private transformCsvDataToFahrbeziehung(arm: KnotenarmDTO): Map<string, Array<ZeitintervallDTO>> {
    const fahrbeziehungen: Map<string, Array<ZeitintervallDTO>> = new Map<string, Array<ZeitintervallDTO>>();
    const zeitinervalleproNach: Map<string, Array<ZeitintervallDTO>> = new Map<string, Array<ZeitintervallDTO>>();
    // Ersten 3 Zeilen entfernen
    arm.filedata.shift();// Metda-Header
    let knotenarmVon: string = arm.filedata.shift()!.split(this.SEPARATOR)[3];
    ...
    arm.filedata.forEach((line: string) => {
      if (line.trim().length === 0) {
        ...
        const startEndeOfIntervallnummer: StartUhrzeitEndeUhrzeit = this.getStartEndeOfIntervallnummer(values[0]);
   ...
   * @private
   */
  private getKeyOfFahrbeziehung(fz: FahrbeziehungDTO, isKreisverkehr: boolean): string {
    let key: string = `${fz.knotenarm}`;
    if (isKreisverkehr) {
      if (fz.hinein) {
        key += 'e';
      } else if (fz.vorbei) {
