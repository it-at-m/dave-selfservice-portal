      ...
      <v-row dense>
        <v-col cols="12" md="10">
          <v-row dense no-gutters>
            <v-col cols="12" md="2">
              <lhm-text-field caption="Kreisverkehr" :text="getKreisverkehrText"/>
            </v-col>
            <v-col cols="12" md="8">
              <lhm-text-field caption="Kreuzungsname" :text="zaehlung.kreuzungsname"/>
            </v-col>
            <v-col cols="12" md="1" v-if="isZaehlungEditable">
              <v-btn @click="fileUpload">
                <v-icon>{{ this.appendIcon }}</v-icon>
                Upload
              </v-btn>
              <v-form ref="fileInputForm">
                <v-file-input :id="FILE_INPUT_FIELD_ID"
                              style="display: none"
                              dense
                              single-line
                              multiple
          ...
          <v-row dense v-for="arm in getKnotenarme" :key="arm.id">
            <v-col cols="12" md="6">
              <v-text-field :value="arm.strassenname"
                            single-line
                            dense
                            readonly
                            color="black"
            ...
            <v-col cols="12" md="5">
              <v-text-field v-model="arm.filename"
                            :style="getStyle(arm)"
                            single-line
                            dense
                            readonly
        ...
        <v-spacer/>
        <v-col cols="12" md="2">
          <v-data-table v-if="isNotKreisverkehr"
                        dense
...
export default class KnotenLageForm extends Vue {
  ...
  private static readonly EXPECTED_META_HEADER: string = "Zählstellennummer;Zählart;Datum;Knotenarmnummer;;;;;"

  private static readonly EXPECTED_ZAEHLDATEN_HEADER: string = "Intervallnummer;nach;Pkw;Lkw;Lz;Bus;Krad;Rad;Fuss"

  private static readonly SEPARATOR: string = ";"

  private readonly FILE_INPUT_FIELD_ID: string = 'fileInputField';

  private resetFileInput: number = 0;

  private zaehlung: ZaehlungDTO = DefaultObjectCreator.createDefaultZaehlungDTO();
  ...
  get allFahrbeziehungen(): Array<FahrbeziehungDTO> {
    return this.zaehlung.fahrbeziehungen.sort(FahrbeziehungComparator.sortByActiveVonAndNach);
  }

  private getStyle(arm: KnotenarmDTO): string {
    let style: string = 'display: none';
  ...
  private onFileSelect(selectedFiles: Array<any>) {
    ...
    if (!_.isNil(selectedFiles) && selectedFiles.length > this.zaehlungStore.knotenarme.length) {
  ...
  private readFiles(selectedFiles: Array<any>) {
    ...
    selectedFiles.forEach(myFile => {
      ...
      fileReader.addEventListener("load", function () {
        ...
        const knotenarmnummerOfCsv: number = that.getKnotenarmnummerOfCsv(csv);
        itemsProcessed++;
        that.zaehlung.knotenarme.forEach((zaehlungArm: KnotenarmDTO) => {
          if (zaehlungArm.nummer === knotenarmnummerOfCsv) {
            // Plausibilitätscheck
            const isPlausible: string = that.checkUploadedFiledata(knotenarmnummerOfCsv, csv);
            ...
            if (itemsProcessed === selectedFiles.length) {
              // Damit nacheinander ein File mit identischem Namen hocheladen werden
              // kann, wird immer der FileInput zurückgesetzt
              that.resetFileInput = Math.floor(Math.random() * 10001);
              if (successfull) {
      ...
      if (myFile) {
        if (this.wrongFileType(myFile)) {
          that.$store.dispatch('snackbar/showToast', {
            ...
            snackbarTextPart2: `Es werden nur CSV-Dateien unterstützt und keine ${myFile.name.split('.').pop().toUpperCase()}-Dateien.`
          });
        } else {
  ...
  public getKnotenarmnummerOfCsv(csvData: Array<string>): number {
    ...
    const metaDataSplitted: Array<string> = metaData.split(KnotenLageForm.SEPARATOR);
   ...
   */
  public checkUploadedFiledata(armNummer: number, csvData: Array<string>): string {
    ...
    const expectedMetaData: string = `${zaehlung.zaehlstelleNummer};${zaehlung.zaehlart === Zaehlart.N ? '' : zaehlung.zaehlart};${zaehlung.datum};${armNummer};;;;;`
    ...
    // ZaehldatenHeader vorhanden und korrekt?
    if (zaehldatenHeader!.trim() !== KnotenLageForm.EXPECTED_ZAEHLDATEN_HEADER) {
    ...
    for (let [csvLineIndex, data] of csvData.entries()) {
      ...
      if (csvLineIndex > 2 && data.trim().length > 0) {
        ...
        const splittedLine: Array<any> = data.split(KnotenLageForm.SEPARATOR);
        ...
        if (csvLineIndex > 3) {
          ...
          if (this.zaehlung.kreisverkehr) {
            fahrbeziehung = this.zaehlung.fahrbeziehungen.find(fahrbeziehung => {
              return fahrbeziehung.knotenarm === armNummer
            ...
            fahrbeziehung = this.zaehlung.fahrbeziehungen.find(fahrbeziehung => {
              return armNummer === fahrbeziehung.von
        ...
        if (this.zaehlungStore.kreisverkehr) {
          ...
          if (splittedLine[1] !== 'e' && splittedLine[1] !== 'v' && splittedLine[1] !== 'a') {
            return `Die 'nach'-Spalte in Zeile ${csvLineNumber} darf nur 'e', 'v' oder 'a' enthalten.\nWar: ${splittedLine}`;
          }
          for (let columnIndex = 2; columnIndex < splittedLine.length; columnIndex++) {
