export default {
  install(Vue: any, options: any) {
    ...
    const RULE_SET: any = {
      ...
      NOSPACES: [
        (v: string) =>
          !v || v.toString().indexOf(" ") < 0 || "Keine Leerzeichen erlaubt",
      ],
      ONLYNUMBERS: [
        (v: string) =>
          !v || /^([0-9]+)$/.test(v) || "Feld darf nur Zahlen enthalten",
      ],
      MAX_SIX_CHARACTERS: [
        (v: string) =>
          !v ||
          v.toString().length <= 6 ||
          "Feld darf nicht mehr als 6 Zeichen beinhalten",
      ],
      SIX_CHARACTERS: [
        (v: string) =>
          !v || v.toString().length == 6 || "Feld muss 6 Zeichen beinhalten",
      ],
      DATE: [
        (v: string) =>
          !v ||
          /^([0-9][0-9]\.[0-9][0-9]\.[0-9][0-9][0-9][0-9])$/.test(v) ||
          "Feld muss das Format DD.MM.YYYY haben",
      ],
      MONEY: [
        (v: string) =>
          !v ||
          /^((\$?([0-9]{1,3}(\.)?([0-9]{3}(\.)?)*[0-9]{3}|[0-9]+)(,[0-9]{1,2})?)|(0,[0-9]{1,2}))$/.test(
            v
          ) ||
          "Feld kann höchstens 2 Nachkommastellen haben z.B. 1234,00 oder 1.234,00",
      ],
      DECIMALPOINTS: [
        (v: string) =>
          !v ||
          /^(([0-9.]*[0-9](,[0-9]{1,2})?)|0,[0-9]{1,2})$/.test(v) ||
      ...
      PHONENUMBER: [
        (v: string) =>
          !v ||
          /^([0(+][0-9\\.-\\/ ()]{7,})$/.test(v) ||
          "Vorwahl beginnend mit 0 ( oder + gefolgt von Leerstellen 0-9 . (  ) - /",
      ],
      EMAIL: [
        (v: string) =>
          !v || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) || "Email ungültig",
      ], // Aufbau: _@_._
      ...
      X_KOORDINATE: [
        (v: string) =>
          !v ||
          /^([0-9]{6})$/.test(v) ||
          "Feld muss einer Koordinatenangabe mit genau 6 Ziffern entsprechen",
      ],
      Y_KOORDINATE: [
        (v: string) =>
          !v ||
          /^([0-9]{7})$/.test(v) ||
          "Feld muss einer Koordinatenangabe mit genau 7 Ziffern entsprechen",
      ],
